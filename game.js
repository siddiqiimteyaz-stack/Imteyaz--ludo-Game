...
function buildBoardDom() {
  ...
  COLORS.forEach(color => {
    ...
    for (let r = rowStart; r < rowStart + 6; r++) {
      for (let c = colStart; c < colStart + 6; c++) {
        getCellDom(r, c).classList.add("yard-" + color);
      }
    }
  });
  ...
  // हर यार्ड के अंदर 4 गोटियों के लिए एक छोटा 2x2 box बनाएं
  COLORS.forEach(color => {
    const { rowStart, colStart } = YARD_QUADRANT[color];
    const centerCellDom = getCellDom(rowStart + 2, colStart + 2);
    const yardBox = document.createElement("div");
    yardBox.className = "yardBox";
    yardBox.id = "yardBox-" + color;
    yardBox.style.gridColumn = `span 2`;
    yardBox.style.gridRow = `span 2`;
    centerCellDom.appendChild(yardBox);
    centerCellDom.style.gridColumn = `span 2`;
    centerCellDom.style.gridRow = `span 2`;
  });
}
